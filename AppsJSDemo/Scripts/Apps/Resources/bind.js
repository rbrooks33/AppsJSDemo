...
define([], function () {
    var Me = {
        ...
        DataBind: function (obj, key, isCollection, callback, changedCallback, boundCallback) {
            ...
            Me.Obj = obj;
            var boundElements = Apps.$("[data-bind-type='" + key + "']");

            Apps.$.each(boundElements, function (index, boundElement) {

                Me.CountDownBind.count++;

                var propertyName = Apps.$(boundElement).attr("data-bind-property"); //For now: required
                var propertyCollectionName = Apps.$(boundElement).attr("data-bind-collection-property"); //Optional if using collection
                var propertyVisible = Apps.$(boundElement).attr("data-bind-property-visible");
                var propertyCollectionVisible = Apps.$(boundElement).attr("data-bind-collection-property-visible");
                var propertyValueIsInt = Apps.$(boundElement).attr("data-bind-int");
                ...
                if (elementType.toLowerCase() === "text") {
                    ...
                    //SET THE VALUE
                    Apps.$(boundElement).val(propertyValue);

                    //WIRE EVENTS
                    Apps.$(boundElement).on('change keyup paste', function () {
                        ...
                        if (changedCallback)
                            changedCallback(Apps.$(boundElement), propertyName, Apps.$(this).val(), collectionIndex);
                    });
                    ...
                    Apps.$(boundElement).text(propertyValue);
                    ...
                    Apps.$(boundElement).on('change', function () {
                        ...
                        eval("Me.Obj." + propertyName + " = '" + Apps.$(this).text() + "'");
                    ...
                    Apps.$(boundElement).on('change', function () {
                        ...
                        if (isIntWire)
                            isCollection ? eval("objWire" + propertyCollectionName + " = " + Apps.$(this).val()) : eval("objWire." + propertyName + " = " + parseInt(Apps.$(this).val()));
                        else
                            isCollection ? eval("objWire" + propertyCollectionName + " = '" + Apps.$(this).val() + "'") : eval("objWire." + propertyName + " = '" + Apps.$(this).val() + "'");
                ...
                //Some elements by nature e.g. radio need logic so just let changed event fire
                    ...
                    if (boundElement.type === 'text') {
                        ...
                        //WIRE EVENTS
                        Apps.$(boundElement).off().on('change keyup paste', function () {
                        ...
                        Apps.$(boundElement).on('change', function () {
                            ...
                            if (changedCallback)
                                changedCallback(Apps.$(boundElement), propertyName, Apps.$(this).prop("checked"), collectionIndex);

                        });

                    }
                    else if (boundElement.type === 'radio') {
                        Apps.$(boundElement).change(function () {
                            if (changedCallback)
                                changedCallback(Apps.$(boundElement), propertyName, Apps.$(this).prop("checked"), collectionIndex);
                        });
                    }
                ...
                if (Me.BindCallback)
                    Me.BindCallback(Apps.$(boundElement), propertyName, propertyValue);

                if (callback)
                    callback(Apps.$(boundElement), propertyName, propertyValue, collectionIndex);
        ...
        DataBindTable: function (settings) {
            ...
            //GET AND CLEAR TABLE
            var table = Apps.$("#" + tableId); //Get from DOM
            if (table.length === 0) {
                //get parent template, if any
                var parentTemplate = settings.template.Selector;
                if (parentTemplate.length === 1)
                    table = Apps.$('<table id="' + settings.tableid + '" class="table"></table>').appendTo(parentTemplate);
            }

            Apps.$.each(table.find("tr"), function (index, child) {
                if (index > 0) //Assumption there is a header row
                    Apps.$(child).detach();
            });

            //APPEND ROWS
            Apps.$.each(settings.data, function (index, row) {
        ...
        BindRow: function (row) {
            ...
            Apps.$.each(props, function (index, prop) {
        ...
        GetTable: function (settings) {
            ...
            */
            ...
            //GET AND CLEAR TABLE
            var table = Apps.$('<table id="' + settings.tableid + '" class="table"></table>');
        ...
        EnableDrag: function (drag) {
            ...
            Apps.$("." + drag.rowclass).mousedown(function (e) {
                var tr = Apps.$(e.target).closest("TR"), si = tr.index(), sy = e.pageY, b = Apps.$(document.body), drag;
                if (si === 0) return;
                ...
                function move(e) {
                    ...
                    tr.siblings().each(function () {
                        var s = Apps.$(this), i = s.index(), y = s.offset().top;
                ...
                function up(e) {
                    ...
                    Apps.$(document).unbind("mousemove", move).unbind("mouseup", up);
                    b.removeClass("grabCursor").css("userSelect", "none");
                    tr.removeClass("grabbed");
                }
                Apps.$(document).mousemove(move).mouseup(up);
            });
